...
% initial guess (these were the result of empirical experimentation with
...
% Define initial conditions also based on MATLAB's defaults in the cftool
n=size(t,1); 
i=0;
tol=.1;
 
while (i<100) && (tol>1E-6)
% Set the vaule of the function and perform u substitution the function we
...
% thought would be appropriate for our problem in this case the one used in
% Gibb's free energy relationships seemed appropriate this is because this
...
% appropriate to our task, we were also able to check it verse the cftool in
...
% Here we store the values of the Jacobian
J=[exp(u) zeros(n,1) ones(n,1)];
 
% Unfortunately since our code didn't work properly in assignment 2 we
